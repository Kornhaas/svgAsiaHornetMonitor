...
async function refreshStatus() { try { const state = await (await fetch('/status')).json(); frameSize = { width: state.frame_width, height: state.frame_height }; indicator.classList.toggle('motion', state.motion); message.textContent = state.camera_error || (state.motion ? 'Motion detected in ROI' : 'Watching ROI'); details.textContent = `Largest moving area: ${state.largest_area}px²${state.last_event ? ` · Latest event: ${state.last_event}` : ''}`; showRoi(state.roi); } catch (_) { message.textContent = 'Status unavailable'; } }
...
async function updateRequest(path) { const response = await fetch(path, { method: 'POST' }); const result = await response.json(); updateMessage.textContent = result.message || (result.state === 'current' ? `Up to date (${result.current}).` : `${result.pending} update(s) available.`); }
