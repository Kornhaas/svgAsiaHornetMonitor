...
function reconnectStream() { if (previewDisabled) return; clearTimeout(reconnectTimer); reconnectTimer = setTimeout(() => { stream.src = `/stream.mjpg?reconnect=${Date.now()}`; }, 1500); }
function setPreviewDisabled(disabled) { if (disabled === previewDisabled) return; previewDisabled = disabled; stream.classList.toggle('d-none', disabled); roiBox.classList.toggle('d-none', disabled); nightPreview.classList.toggle('d-none', !disabled); if (disabled) stream.removeAttribute('src'); else stream.src = `/stream.mjpg?reconnect=${Date.now()}`; }
async function refreshStatus() { try { const state = await (await fetch('/status', { cache: 'no-store' })).json(); frameSize = { width: state.frame_width, height: state.frame_height }; const night = state.night_mode || {}; setPreviewDisabled(Boolean(night.preview_disabled)); indicator.classList.toggle('motion', state.motion); message.textContent = state.camera_error || (night.preview_disabled ? (night.active ? 'Night mode active' : 'Night mode pending') : (state.motion ? 'Motion detected in ROI' : 'Watching ROI')); details.textContent = `Largest moving area: ${state.largest_area}px²${state.last_event ? ` · Latest event: ${state.last_event}` : ''}`; showRoi(state.roi); } catch (_) { message.textContent = 'Status unavailable'; } }
...
async function updateRequest(path) { const response = await fetch(path, { method: 'POST' }); const result = await response.json(); if (!response.ok || result.error) { updateMessage.textContent = result.error || 'Update request failed.'; return; } updateMessage.textContent = result.message || (result.state === 'current' ? `Up to date (${result.current}).` : `${result.pending} update(s) available.`); if (path === '/updates/install' && result.state === 'running') { updateMessage.textContent = 'Update installed; reconnecting the monitor…'; stream.removeAttribute('src'); setTimeout(() => window.location.reload(), 8000); } }
